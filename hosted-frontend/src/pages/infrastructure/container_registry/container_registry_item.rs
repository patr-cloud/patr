		...
		>
			<A href="some">
				<td class="flex-col-5 fr-ct-ct">
					<span class="of-hidden txt-of-ellipsis w-35">{stored_item.with_value(|item| item.name.clone())}</span>
				</td>
				<td class="flex-col-2 fr-ct-ct">{stored_item.with_value(|item| item.size.clone())}</td>
				<td class="flex-col-4 fr-ct-ct">{stored_item.with_value(|item| item.date_created.clone())}</td>
