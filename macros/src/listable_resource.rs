...
pub fn parse(input: TokenStream) -> TokenStream {
	...
	let (search_struct_fields, search_struct_field_names) = match search_struct_fields {
		Ok(fields) => fields,
		Err(err) => return err.into(),
	};
	...
	quote::quote! {
		...
		impl models::utils::ListableResource for #ident {
			...
			type SearchStruct = #search_struct_name;
		}
		...
		impl models::utils::IsEmpty for #search_struct_name {
			fn is_empty(&self) -> bool {
				#(self. #search_struct_field_names .is_none()) && *
			}
		}
	}
	.into()
