...
const ManageWorkspace = () => {
    ...
    const [workspaceMembers, { refetch: refetchMembers }] = createResource(resourceParamsWorkspace, async ([auth, id]) => {
        if (!auth || auth.type !== "LoggedIn" || id === "") {
            return;
        }
        ...
        const userDetailsPromises = Object.keys(response.data.users).map(async (userId) => {
            ...
            if (userResponse.ok) {
                ...
                const firstName = user.firstName || (user as any).basic_user_info?.firstName || "";
                const lastName = user.lastName || (user as any).basic_user_info?.lastName || "";
                const username = user.username || (user as any).basic_user_info?.username || "";
            ...
            <PageContainerBody class="flex flex-col justify-between gap-8">
                <div class="flex flex-col gap-6">
                    <div class="flex flex-col gap-4">
                        ...
                        <Show when={!workspaceMembers.loading && workspaceMembers() && workspaceMembers()!.length > 0}>
                            <div class="mt-4">
                                <div class="text-white text-sm font-medium mb-2">Workspace Members</div>
                                <Table<Record<string, unknown>>
                                    column_grids={["flex-2", "flex-1"]}
                                    headings={["User", "Roles"]}
                                    rows={workspaceMembers()! as unknown as Record<string, unknown>[]}
                                    renderRow={(member) => {
