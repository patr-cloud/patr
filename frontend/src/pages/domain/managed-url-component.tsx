    ...
    <>
      {openEdit() ? (
        ...
        <tr class="table-row cursor-auto">
          ...
          <td class="flex-3 flex items-center justify-center">
            {domainTypeToTitle(props.managedUrl.type)}
          </td>
          ...
          <td class="flex-[0.3] flex items-center justify-center">
            <div class="flex gap-2 items-center">
              {!props.managedUrl.isConfigured && (
                <InfoPopup
                  triggerIcon={() => (
                    <FiAlertCircle color="text-warning" size={18} />
                  )}
                  title="Configuration Required"
                  content={() => (
                    <div>
                      <p class="text-gray-300 text-sm mb-2">
                        This managed URL is not properly configured. Please
                        update your DNS settings to point to our servers.
                      </p>

                      <div class="bg-black/30 p-2 rounded text-xs text-gray-400 mb-2">
                        <p>Type: CNAME</p>
                        <p>
                          Name: {props.managedUrl.subDomain}.
                          {props.domainInfo.name || ""}
                        </p>
...
const ManagedUrlComponent = (props: ManagedUrlComponentProps) => {
  ...
  const [urlType, setUrlType] = createSignal<urlTypeT>(
    props.managedUrl.type as urlTypeT
  );
  const [target, setTarget] = createSignal<string | null>(
    props.managedUrl.type === "proxyDeployment"
      ? props.managedUrl.deploymentId
      : null
  );
  ...
  return (
    <form
      class="w-full mb-2 p-lg bg-secondary-light rounded-xs"
      onSubmit={onSubmit}
    >
