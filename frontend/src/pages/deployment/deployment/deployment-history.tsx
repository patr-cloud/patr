...
const DeploymentHistory = () => {
    ...
    const [deploymentHistory] = createResource(resourceParamsDeploymentHistory, async ([auth, wsId, id]) => {
        if (!wsId || !auth || auth.type !== "LoggedIn" || id === "") {
            console.log(
                "Invalid parameters for fetching deployment history",
                wsId,
                auth,
                id
            );
        ...
        const resposne = await httpRequest<ListDeploymentDeployHistoryResponse>(
        ...
        if (!resposne.ok) {
            console.error("Failed to fetch deployment history:", resposne.data.error);
            toast("Failed to fetch deployment history", "error");
            return undefined;
        }
        console.log("Fetched deployment history:", resposne.data);
                ...
                <Table
                    ...
                    renderRow={(deploy) => (
                        <tr class="table-row">
                            ...
                            <td class="flex-1">
                                {deploy.created.toLocaleString().split(".")[0]}
                            </td>
                        </tr>
                    )}
                />
