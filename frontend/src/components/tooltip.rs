	...
	create_effect(cx, move |_| {
		...
		let handle_mouse_move = move |e: ev::MouseEvent| {
			if let Some((tooltip_ref, parent_ref)) =
				tooltip_ref.get().zip(parent_ref.get())
			{
				if is_visible.get() {
