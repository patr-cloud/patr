...
pub fn SignUpPage() -> impl IntoView {
	...
	// 		.get()
	// 	},
	// 	move |username: String| async move {
		...
		<div class="box-onboard text-white">
			...
			>
				...
				<Input
					...
					start_icon={|| Icon(IconProps::builder().icon(IconType::User).build())}
					value={username}
				...
				<Input
					...
					start_icon={|| Icon(IconProps::builder().icon(IconType::Mail).build())}
					value={email}
				...
				<div class="fr-fe-ct w-full mt-lg">
					{app_type
						...
						.then(|| {
							view! {
								<Link class="btn mr-xs" to="/confirm">
									"ALREADY HAVE AN OTP"
								</Link>
							}
								.into_view()
					...
					<Show
						...
						fallback={move || {
							view! {
								<Link variant={LinkStyleVariant::Contained} >
									"LOADING..."
								</Link>
							}
						}}
