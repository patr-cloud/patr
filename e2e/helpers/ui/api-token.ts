...
export async function readNewTokenFromModal(page: Page): Promise<string> {
  await expect(page.getByText(/API Token Created Successfully/i)).toBeVisible({ timeout: 15_000 });
  // The token text starts with "patrv1." — locate by that prefix.
  const token = await page.locator('text=/patrv1\\.[a-f0-9-]+\\.[a-f0-9-]+/i').first().innerText();
  return token.trim();
}
