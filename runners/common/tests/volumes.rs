...
/// Actually executes `m006_deployment_volumes`. A fresh database marks every
/// migration applied without running it, so this rebuilds the pre-m006
/// tables by hand, un-marks the migration, and runs the migration runner.
#[tokio::test]
async fn m006_replaces_the_volume_tables() {
	let setup = setup().await;
	let mut conn = setup.database.acquire().await.unwrap();
	...
	.await
	.unwrap();
	sqlx::query("DELETE FROM migrations WHERE name = 'm006_deployment_volumes'")
		.execute(&mut *conn)
		.await
		.unwrap();
