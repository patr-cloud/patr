...
async fn migrate(connection: &mut DatabaseConnection) -> Result<(), ErrorType> {
	...
	// Migrate existing custom hostnames: for each distinct (sub_domain,
	...
	sqlx::query(
		...
		SELECT DISTINCT
			...
			workspace_domain.cloudflare_custom_hostname_id,
			TRUE
		FROM
			managed_url
		INNER JOIN
			workspace_domain ON workspace_domain.id = managed_url.domain_id
		WHERE
			managed_url.deleted IS NULL AND
			workspace_domain.deleted IS NULL AND
			workspace_domain.cloudflare_custom_hostname_id IS NOT NULL
	...
	// Drop the old column from workspace_domain
	sqlx::query(
		r#"
	...
	// Drop is_active from managed_url
	sqlx::query(
		r#"
	...
	// Add FK from managed_url to managed_url_custom_hostname
	sqlx::query(
		r#"
		ALTER TABLE managed_url
