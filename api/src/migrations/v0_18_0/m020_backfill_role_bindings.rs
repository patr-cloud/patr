...
//!
//! 1. Abort if any role's permissions disagree about their resources — a binding carries one scope
//!    for the whole role, so such a role has no representation and the operator has to split it
//!    first.
//! 2. Roles named after a frozen default are reset to it and marked immutable.
//! 3. `role_permission`, the flat permission list, from the legacy type rows.
//! 4. A binding per assignment: `Exclude(∅)` → one workspace-scope binding; `Include(S)` → one per
...
//! 5. Token ceilings, expanded from the legacy per-permission resource lists by those same three
//!    rules.
//!
