...
pub(super) async fn migrate(
	...
	add_deployment_check_status_permission(connection, config).await?;
...
async fn add_deployment_check_status_permission(
	connection: &mut <Database as sqlx::Database>::Connection,
	_config: &Settings,
) -> Result<(), Error> {
	for &permission in
		["workspace::infrastructure::deployment::check_status"].iter()
	{
		let uuid = loop {
