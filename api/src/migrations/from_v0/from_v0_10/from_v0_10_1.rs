...
pub(super) async fn migrate(
	...
	add_permission_for_metrics(connection, config).await?;
...
async fn add_permission_for_metrics(
	connection: &mut <Database as sqlx::Database>::Connection,
	_config: &Settings,
) -> Result<(), Error> {
	let permission = "workspace::infrastructure::metrics::get";
	let uuid = loop {
...
async fn reset_permission_order(
	...
	for permission in [
		...
		// metrics
		"workspace::infrastructure::metrics::get",
		// workspace
		"workspace::edit",
		"workspace::delete",
	] {
