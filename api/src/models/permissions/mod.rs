...
mod api_token;
/// Contains the functions to extract permissions for an OAuth access token.
mod oauth;
...
///
/// OAuth access tokens are recognised by their signature algorithm and handled
/// before the client type is consulted, because they are valid on both arms:
/// the cloud build mounts the API as [`ClientType::ApiToken`], while the
/// self-hosted build mounts the whole thing as [`ClientType::WebDashboard`].
/// `api_allowed` is the endpoint's `API_ALLOWED`, and it is the only thing
/// keeping an OAuth grant off the endpoints that change how a user logs in —
/// [`mount_endpoint`][1] filters those out at mount time only for API tokens.
///
/// [1]: crate::utils::extensions::RouterExt::mount_endpoint
pub async fn get_user_data_for_token(
	...
	if oauth::is_oauth_access_token(token) {
		if !api_allowed {
			warn!("An OAuth access token was presented to an endpoint that forbids API clients");
			return Err(ErrorType::ClientTypeNotAllowed);
		}

		return oauth::get_permissions(database, redis, config, client_ip, token).await;
	}
...
/// Compute a user's current permission map and cache it under `login_id`.
///
/// On a valid cache hit the cached map is returned directly; otherwise the
/// user's role-derived permissions are read from the database (workspace
/// ownership + per-role includes/excludes for the workspaces they're a
/// member of) and written to the cache before being returned.
///
/// Shared by web sessions and OAuth grants, which both act with the user's
/// full current authority — unlike an API token, which intersects this with
/// its own ceiling. The cache is keyed on the login rather than the user so
/// that revoking one session or grant does not disturb the others.
#[tracing::instrument(skip(db_connection, redis_connection))]
pub async fn get_current_permissions_for_user(
	db_connection: &mut DatabaseConnection,
	redis_connection: &mut RedisClient,
	login_id: &Uuid,
	user_id: &Uuid,
) -> Result<BTreeMap<Uuid, WorkspacePermission>, ErrorType> {
	if let Some(cached) = get_cached_permissions(redis_connection, login_id, user_id).await? {
		return Ok(cached);
	}
