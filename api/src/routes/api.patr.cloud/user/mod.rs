...
pub async fn setup_routes(state: &AppState) -> Router {
	Router::new()
		...
		.mount_auth_json_endpoint(change_password, state)
		.mount_auth_json_endpoint(get_user_details, state)
		.mount_auth_json_endpoint(get_user_info, state)
		.mount_auth_json_endpoint(list_workspaces, state)
		.mount_auth_json_endpoint(update_user_info, state)
}
