...
pub async fn setup_routes(state: &AppState, host_client_types: &[ActorClientType]) -> Router {
	Router::new()
		.mount_endpoint(social_login_initiate, state, host_client_types)
		.mount_endpoint(social_login_callback, state, host_client_types)
		.mount_endpoint(social_login_setup, state, host_client_types)
}
