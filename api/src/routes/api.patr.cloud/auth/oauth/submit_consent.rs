...
use super::{append_query_params, error::OAuthErrorCode};
use crate::{
	models::oauth::types::{OAuthAuthorizationCode, OAuthAuthorizationRequest, hash_code},
	prelude::*,
};
...
pub async fn submit_consent(
	...
	if approved {
		...
		redis
			.setex(
				redis::keys::oauth_authorization_code(&hash_code(&code)),
		...
		params.push(("error", OAuthErrorCode::AccessDenied.as_str().to_owned()));
		params.push((
			"error_description",
			"The user denied the request".to_owned(),
		));
	}
...
/// Mints an authorization code: 32 random bytes, base64url without padding.
///
/// A v4 UUID would carry 122 bits of entropy; OAuth 2.1 asks for at least
/// 128, so this takes bytes from the OS directly rather than reusing the
/// UUID helper used elsewhere.
