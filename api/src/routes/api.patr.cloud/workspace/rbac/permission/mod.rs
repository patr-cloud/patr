...
pub async fn setup_routes(state: &AppState, host_client_types: &[ActorClientType]) -> Router {
	Router::new()
		.mount_auth_endpoint(get_current_permissions, state, host_client_types)
		.mount_auth_endpoint(list_all_permissions, state, host_client_types)
		.mount_auth_endpoint(list_all_resource_types, state, host_client_types)
