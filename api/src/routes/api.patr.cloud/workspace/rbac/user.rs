...
pub async fn setup_routes(state: &AppState) -> Router {
	Router::new()
		.mount_auth_endpoint(add_user_to_workspace, state)
...
async fn add_user_to_workspace(
	AuthenticatedAppRequest {
		...
		config,
		user_data
	}: AuthenticatedAppRequest<'_, AddUserToWorkspaceRequest>,
) -> Result<AppResponse<AddUserToWorkspaceRequest>, ErrorType> {
	info!("Starting: Get current permissions");

	// LOGIC

	AppResponse::builder()
		.body(AddUserToWorkspaceResponse)
		.headers(())
...
async fn list_users_with_roles_in_workspace(
	...
	AppResponse::builder()
		.body(ListUsersWithRolesInWorkspaceResponse {
			users: todo!(),
		})
		.headers(())
		.status_code(StatusCode::OK)
