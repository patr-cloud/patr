...
pub fn create_sub_app(
	...
	// Cancel request for Transfer user controller domain to patr controller
	// domain
	app.put(
		"/:domainId/cancel-transfer",
		[
			EveMiddleware::ResourceTokenAuthenticator(
				permissions::workspace::domain::VERIFY,
				api_macros::closure_as_pinned_box!(|mut context| {
			...
			EveMiddleware::CustomFunction(pin_fn!(
				cancel_transfer_domain_to_patr
			)),
		],
	);
...
async fn cancel_transfer_domain_to_patr(
	mut context: EveContext,
	_: NextHandler<EveContext, ErrorData>,
) -> Result<EveContext, Error> {
	let request_id = Uuid::new_v4();
	log::trace!(
		"request_id: {} - Cancelling transfer domain to patr",
		request_id
	);
	...
	service::cancel_transfer_domain_to_patr(
		context.get_database_connection(),
		&domain_id,
		&request_id,
	)
