...
pub async fn setup_routes(state: &AppState, host_client_types: &[ActorClientType]) -> Router {
	Router::new()
		.merge(container_registry::setup_routes(state, host_client_types).await)
		.merge(deployment::setup_routes(state, host_client_types).await)
		.merge(domain::setup_routes(state, host_client_types).await)
		.merge(managed_url::setup_routes(state, host_client_types).await)
		.merge(rbac::setup_routes(state, host_client_types).await)
		.merge(runner::setup_routes(state, host_client_types).await)
		// .merge(secret::setup_routes(state, host_client_types).await)
		.merge(service_account::setup_routes(state, host_client_types).await)
		.merge(volume::setup_routes(state, host_client_types).await)
		.mount_auth_endpoint(create_workspace, state, host_client_types)
		.mount_auth_endpoint(delete_workspace, state, host_client_types)
		.mount_auth_endpoint(get_resources_info, state, host_client_types)
		.mount_auth_endpoint(get_workspace_info, state, host_client_types)
		.mount_auth_endpoint(leave_workspace, state, host_client_types)
		.mount_auth_endpoint(is_name_available, state, host_client_types)
		.mount_auth_endpoint(update_workspace_info, state, host_client_types)
}
