...
pub fn create_sub_route(app: &App) -> Router {
	...
	router.route("/auth", get(connect_to_github));

	router.route("/auth-callback", post(github_oauth_callback));

	router.route("/repo/sync", post(sync_repositories));

	router.route("/repo", get(list_repositories));

	router.route("/repo/:repoId", get(get_repo_info));

	router.route("/repo/:repoId/activate", post(activate_repo));

	router.route("/repo/:repoId/deactivate", post(deactivate_repo));

	router.route("/repo/:repoId/build", get(get_build_list));

	router.route("/repo/:repoId/build/:buildNum", get(get_build_info));
	...
	router.route("/repo/:repoId/build/:buildNum/cancel", post(cancel_build));

	router.route("/repo/:repoId/build/:buildNum/restart", post(restart_build));

	router.route(
		"/repo/:repoId/branch/:branchName/start",
		post(start_build_for_branch),
	);

	router.route("/repo/:repoId/ref", get(list_git_ref_for_repo));

	router.route("/repo/:repoId/patr-ci-file/:gitRef", get(get_patr_ci_file));

	router.route("/repo/:repoId/patr-ci-file", post(write_patr_ci_file));

	router.route("/sign-out", delete(sign_out));
