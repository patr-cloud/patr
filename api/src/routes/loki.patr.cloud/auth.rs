...
/// Shared authentication and permission checking for both Loki and OTLP
/// handlers. Takes pre-extracted credentials. Returns `(runner_id,
/// workspace_id)` on success, or an error Response.
pub(super) async fn authenticate_and_authorize(
	...
	// Loki push is only allowed from runners (service accounts), not from users
