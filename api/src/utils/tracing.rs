...
/// Sets up the OpenTelemetry tracing and logging pipelines based on the
/// provided configuration. This function initializes the OpenTelemetry
/// exporters for both logs and spans, configures the tracing subscriber with
/// appropriate layers for console output and OpenTelemetry integration, and
/// sets the global tracer provider for the application. The function returns
/// the logger and tracer providers, which can be used to flush and shut down
/// the pipelines when the application exits.
pub fn setup_tracing(config: &AppConfig) -> (SdkLoggerProvider, SdkTracerProvider) {
...
/// Flushes the tracing and logging pipelines. This should be called before the
/// application exits to ensure that all logs and spans are exported before the
/// application shuts down.
pub fn flush_tracing(logger_provider: SdkLoggerProvider, tracer_provider: SdkTracerProvider) {
