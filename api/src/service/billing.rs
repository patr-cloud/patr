...
pub async fn calculate_total_bill_for_workspace_till(
	...
	let total_resource_usage_bill = ResourceUsageBill {
		...
		managed_url_cost: format!("{:.2}", managed_url_cost),
		docker_repo_cost: format!("{:.2}", docker_repo_cost),
		managed_domain_cost: format!("{:.2}", managed_domain_cost),
		managed_secret_cost: format!("{:.2}", managed_secret_cost),
		deployment_usages,
		database_usages,
		static_sites_usages,
		managed_url_usages,
