...
pub async fn update_kubernetes_deployment(
	...
	// This is because if a user wanted to delete the volume from there sts then
	// a sts will be converted to deployment
	log::trace!(
...
pub async fn update_kubernetes_statefulset(
	...
	// Create the service defined above
	log::trace!("request_id: {} - creating Headless service", request_id);
	let service_api =
		Api::<Service>::namespaced(kubernetes_client.clone(), namespace);
	...
	log::trace!("request_id: {} - statefulset created", request_id);
	...
	// This is because if a user wanted to add volume to their deployment, it
	// becomes a sts
	log::trace!(
		"request_id: {} - deleting the deployment if there is any",
		request_id
	);
