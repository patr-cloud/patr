...
#[typed_path("/workspace/:workspace_id/audit-log/:resource_id")]
pub struct GetResourceAuditLogPath {
...
pub struct GetResourceAuditLogRequest;

impl ApiRequest for GetResourceAuditLogRequest {
	const METHOD: Method = Method::GET;
	const IS_PROTECTED: bool = true;

	type RequestPath = GetResourceAuditLogPath;
	type RequestQuery = Paginated;
	type RequestBody = ();
	type Response = GetResourceAuditLogResponse;
}

#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "camelCase")]
pub struct GetResourceAuditLogResponse {
...
mod test {
	...
	use super::{GetResourceAuditLogRequest, GetResourceAuditLogResponse};
	use crate::{
	...
	fn assert_request_types() {
		assert_tokens(
			&GetResourceAuditLogRequest,
			&[Token::UnitStruct {
				name: "GetResourceAuditLogRequest",
			}],
		);
	}
	...
	fn assert_response_types() {
		assert_tokens(
			...
			&[
				Token::Struct {
					name: "GetResourceAuditLogResponse",
					len: 1,
				},
				Token::Str("auditLogs"),
	...
	fn assert_success_response_types() {
		assert_tokens(
			&ApiResponse::success(GetResourceAuditLogResponse {
				audit_logs: vec![WorkspaceAuditLog {
					id: Uuid::parse_str("2aef18631ded45eb9170dc2166b30867")
						.unwrap(),
					date: Utc.timestamp_opt(1431648000, 0).unwrap().into(),
