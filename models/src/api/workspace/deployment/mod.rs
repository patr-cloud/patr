...
/// Rejects a config mount and a volume at the same path — Docker would refuse
/// the duplicate mount point at deploy time, so catch it at create time.
/// Nested paths are fine. Path shape is enforced by the `deployment_volume`
/// CHECK constraint.
///
/// # Errors
/// Returns an error if any path is both a config mount and a volume.
