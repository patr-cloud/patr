...
/// Args for `patr registry build`.
#[derive(Debug, Clone, ClapArgs)]
pub struct Args {
	/// Docker build context (defaults to the current directory).
	#[arg(default_value = ".")]
	pub context: PathBuf,
	/// Path to the Dockerfile. Forwarded to `docker build -f`. Leave unset
	/// to let docker use `<context>/Dockerfile`.
	#[arg(short = 'f', long = "file")]
	pub file: Option<PathBuf>,
	/// Target Patr repository (name or id). Prompted if omitted.
	#[arg(short = 'r', long = "repo")]
	pub repo: Option<String>,
	/// Destination tag. Defaults to interactive prompt for `latest`.
	#[arg(short = 't', long = "tag")]
	pub tag: Option<String>,
	/// Push after a successful build without prompting. Without this flag,
	/// you're asked in a terminal; in non-TTY the image is built and left
	/// locally.
	#[arg(long)]
	pub push: bool,
	/// Forwarded to `docker build --platform`.
	#[arg(long)]
	pub platform: Option<String>,
	/// Forwarded to `docker build --build-arg`. Repeatable.
	#[arg(long = "build-arg")]
...
pub(super) async fn execute(
	...
	let status = cmd.status().map_err(|e| match e.kind() {
		std::io::ErrorKind::NotFound => {
			AppError::RunnerError("docker not found on PATH. Make sure docker is installed.".to_string())
		}
		_ => AppError::RunnerError(format!("Failed to run `docker build`: {e}")),
	})?;
	if !status.success() {
	...
	let should_push = if args.push {
		true
	} else if std::io::stdin().is_terminal() {
		Confirm::new("Do you want to push this image to the registry?")
			.with_default(true)
			.prompt()
			.expect_tty("Failed to read push confirmation")
	} else {
		eprintln!(
			"Built `{target}` locally. Re-run with `--push` (or in a terminal) to publish it."
		);
		false
	};
	...
	if should_push {
		...
		CommandOutput::builder()
			.text(format!("Built {target} (not pushed)"))
			.json(Value::String(target))
			.build()
			.into_result()
	}
